...
{
	//-------------------------------------------------------------------------
	public class FreeOnRail: MonoBehaviour
	{
		...
		private float flyDist = 0f;
		private Vector3 flyStartLocation;
		private Vector3 flyEndLocation;
		...
		//-------------------------------------------------
		private static FreeOnRail _instance;
		public static FreeOnRail instance
		{
			get
			{
				if (_instance == null)
				{
					_instance = GameObject.FindObjectOfType<FreeOnRail>();
				}
		...
		void Update()
		{
            ...
            else   //flying
            {
				...
				//float distCovered = currentFlyTime * speed;
				//float fractionOfJourney = distCovered / flyDist;
            }
		}
		...
		private void InitiateFly()
		{
			...
			Vector3 flyPosition = pointedAtPosition;

			if (teleportPoint != null)
			{
				flyPosition = teleportPoint.transform.position;
			...
			if (teleportArea != null)
			{
				...
				{
					RaycastHit raycastHit;
					if (Physics.Raycast(flyPosition + 0.05f * Vector3.down, Vector3.down, out raycastHit, floorFixupMaximumTraceDistance, floorFixupTraceLayerMask))
					{
						flyPosition = raycastHit.point;
					}
				}
			}
			...
			if (teleportingToMarker.ShouldMovePlayer())
			{
				...
				flyStartLocation = player.trackingOriginTransform.position;
				flyEndLocation = flyPosition + playerFeetOffset;
				flyDist = Vector3.Distance(flyStartLocation, flyEndLocation);
		...
		private void FlyPlayer()
		{
            ...
            {
				newPosition = Vector3.Lerp(flyStartLocation, flyEndLocation, Mathf.SmoothStep(0.0f, 1.0f, fractionOfJourney));
			}

			else
            {
				newPosition = Vector3.Lerp(flyStartLocation, flyEndLocation, fractionOfJourney);
			}
